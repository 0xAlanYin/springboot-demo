package org.example.elasticsearch.servcie.impl;
...
public class BookServiceIml implements BookService {
  ...
  public BookServiceIml(
      BookRepository bookRepository, ElasticsearchTemplate elasticsearchTemplate) {
  ...
  @Override
  public List<Book> findByTitleAndAuthor(String title, String authorName) {
    // 创建一个布尔查询，要求同时匹配标题和作者
    Query query =
        QueryBuilders.bool(
            builder ->
                builder.must(
                    match(queryAuthor -> queryAuthor.field("authorName").query(authorName)),
                    match(queryTitle -> queryTitle.field("title").query(title))));

    // 使用ElasticsearchTemplate执行查询，并返回匹配的书籍列表
    return elasticsearchTemplate
        .search(NativeQuery.builder().withQuery(query).build(), Book.class)
        .stream()
        // 提取搜索结果中的书籍内容
  ...
  public Book create(Book book) throws DuplicateIsbnException {
    ...
    throw new DuplicateIsbnException(
        String.format("The provided ISBN：%s already exists.Use update instead!", book.getIsbn()));
  }
  ...
  public Book update(String id, Book book) throws BookNotFoundException {
    Book oldBook =
        bookRepository
            .findById(id)
            .orElseThrow(
                () -> new BookNotFoundException("There is not book associated with the given id"));
